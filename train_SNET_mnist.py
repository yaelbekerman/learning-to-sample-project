...
parser.add_argument('--log_dir', default='log/SNET32mnist', help='Log dir [default: log/SNET64]')
parser.add_argument('--part_of_data', type=float, default=1, help='part of data to train on - 1 means all data')
...
PART_DATA = FLAGS.part_of_data
NUMBER_OF_FILES = int(PART_DATA * 30)
FILES_INDEXES = np.random.permutation(np.arange(30))[0:NUMBER_OF_FILES]
print(FILES_INDEXES)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(FILES_INDEXES)):
        log_string('----' + str(fn) + '-----')
        current_data, current_label = provider_mnist.loadDataFile(os.path.join('mnist/', TRAIN_FILES[FILES_INDEXES[fn]]))
