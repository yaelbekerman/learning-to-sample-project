...
parser.add_argument('--log_dir', default='log/SNET32UNSTHRmnisto7', help='Log dir [default: log/SNET64]')
parser.add_argument('--unsupervised_threashold', type=float, default=0.7, help='threashold for unsupervised learning with threashold')
...
NUM_CLASSES = 10
UNSUPERVISED_LABEL_THREASHOLD = FLAGS.unsupervised_threashold
...
def train():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            for pc_size in pc_sizes:
                with tf.variable_scope('classifier' + str(pc_size)):
                    if pc_size == NUM_IN_POINTS:
                        ...
                        loss_classifier = CLASSIFIER_MODEL.get_loss(new_pred, unsupervised_labels_pl, end_points)
            ...
            correct = tf.equal(tf.argmax(new_pred, 1), tf.to_int64(labels_pl))
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(TRAIN_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data, current_label = provider.loadDataFile(os.path.join('mnist/', TRAIN_FILES[train_file_idxs[fn]]))
        ...
        for batch_idx in range(num_batches):
            ...
            if is_training_classifier:
                ...
                summary, step, _, loss_val, pred_val, new_pred_val, unsupervised_current_labels_pl = sess.run(
                    [ops['merged'], ops['step'], ops['train_op'], ops['loss'], ops['pred'], ops['new_pred'],
                    ops['unsupervised_labels_pl']], feed_dict=feed_dict)

            train_writer.add_summary(summary, step)
            for i in range(BATCH_SIZE):
                pred_val_percent = np.max(np.exp(new_pred_val[i]) / (np.sum(np.exp(new_pred_val[i])) + sys.float_info.epsilon))

            new_pred_val = np.argmax(new_pred_val, 1)
            unsupervised_correct = np.sum(new_pred_val == unsupervised_current_labels_pl)
            correct = np.sum(new_pred_val == current_label[start_idx:end_idx])
...
def eval_one_epoch(sess, ops):
    ...
    for fn in range(len(TEST_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data, current_label = provider.loadDataFile(os.path.join('mnist/', TEST_FILES[fn]))
        current_data = current_data[:, :, (0, 2, 3)]
        current_data = current_data[:, 0:NUM_IN_POINTS, :]
        current_label = np.squeeze(current_label)
